...
fn cmd_skill(
    ...
    match command {
        ...
        SkillCommands::Init {
            ...
            printer.newline();
            printer.heading(&format!("Initialize skill: {name}"));
            printer.newline();

            if dry_run {
                printer.dim(&format!("[dry-run] Would create: {}/", skill_dir.display()));
                printer.dim("[dry-run]   skill.toml");
                if skill_type == "template" {
                    printer.dim("[dry-run]   templates/main.tmpl");
                }
                printer.newline();
                return Ok(());
            }
            ...
            match skill_type.as_str() {
                "template" => {
                    std::fs::create_dir_all(skill_dir.join("templates"))
                        .context("could not create skill directory")?;

                    let manifest = TEMPLATE_SKILL_TOML.replace("SKILL_NAME", &name);

                    std::fs::write(skill_dir.join("skill.toml"), manifest)
                        .context("could not write skill.toml")?;

                    let template = "# Generated by forge skill: {{ name }}\n\n\
                        TODO: add your template content here\n";
                    std::fs::write(skill_dir.join("templates/main.tmpl"), template)
                        .context("could not write template")?;
                }
                "transform" => {
                    std::fs::create_dir_all(&skill_dir)
                        .context("could not create skill directory")?;

                    let manifest = TRANSFORM_SKILL_TOML.replace("SKILL_NAME", &name);
                ...
                other => {
                    printer.error(&format!(
                        "Unknown skill type: '{other}'. Use 'template' or 'transform'."
                    ));
                    return Ok(());
                }
            }

            printer.success(&format!("  Created: {}/", skill_dir.display()));
            println!("    skill.toml");
            if skill_type == "template" {
                println!("    templates/main.tmpl");
            }
            printer.newline();
            printer.dim("  Next steps:");
            printer.dim(&format!("    1. Edit {}/skill.toml", skill_dir.display()));
            if skill_type == "template" {
                printer.dim(&format!(
                    "    2. Edit {}/templates/main.tmpl",
                    skill_dir.display()
                ));
            }
            printer.dim(&format!("    3. forge skill link {}", skill_dir.display()));
            printer.dim(&format!("    4. forge skill run {name} --dry-run"));
            printer.newline();
        }

        SkillCommands::Link { path } => {
            let source = std::fs::canonicalize(PathBuf::from(&path))
                .context("could not resolve skill path")?;

            if !source.join("skill.toml").exists() {
                printer.error(&format!("No skill.toml found in {}", source.display()));
                return Ok(());
            }
            ...
            printer.newline();
            printer.heading(&format!("Link skill: {skill_name}"));
            printer.newline();
            println!("  {} → {}", link_target.display(), source.display());
            printer.newline();

            if link_target.exists() {
                if link_target.is_symlink() {
                    let existing_target = std::fs::read_link(&link_target).unwrap_or_default();
                    if existing_target == source {
                        printer.dim("  Already linked to the same path.");
                        printer.newline();
                        return Ok(());
                    }
                    ...
                    if !printer.confirm("Replace existing link?")? {
                        printer.dim("  Aborted.");
                        return Ok(());
                    }
                    ...
                    printer.dim("  Remove it manually before linking.");
                    printer.newline();
                    return Ok(());
                }
            }
            ...
            if dry_run {
                printer.dim("[dry-run] Symlink would be created.");
                printer.newline();
                return Ok(());
            }

            // Create user_dir if needed
            std::fs::create_dir_all(user_dir).context("could not create skills directory")?;
            ...
            #[cfg(not(unix))]
            {
                printer.error("Symlinks are only supported on Unix systems.");
                return Ok(());
            }

            printer.success(&format!("  Linked: {skill_name}"));
            printer.dim("  Skill will appear in `forge skill list` and `forge skill doctor`.");
            printer.newline();
        }
        ...
        SkillCommands::Run { name, input, yes } => {
            ...
            // Parse inputs and prompt for missing required ones
            let mut inputs = parse_skill_inputs(&input)?;
            resolve_skill_inputs(printer, skill, &mut inputs)?;
...
/// Returns an error listing missing inputs if not interactive.
fn resolve_skill_inputs(
    printer: &Printer,
    skill: &DiscoveredSkill,
    provided: &mut HashMap<String, String>,
) -> Result<()> {
    let input_defs = match &skill.manifest.skill.inputs {
        Some(inputs) => inputs,
        None => return Ok(()), // No inputs defined
    };

    let mut missing: Vec<(&String, &forge_core::skills::metadata::InputDef)> = input_defs
        .iter()
        .filter(|(key, def)| def.required && !provided.contains_key(key.as_str()))
        .collect();
    ...
    missing.sort_by_key(|(k, _)| k.to_string());

    if !printer.is_interactive() {
        let names: Vec<&str> = missing.iter().map(|(k, _)| k.as_str()).collect();
        return Err(anyhow::anyhow!(
            "Missing required inputs: {}. Use --input key=value for each.",
            names.join(", ")
        ));
    }

    // Interactive prompt for each missing input
    for (key, def) in &missing {
        let prompt = if let Some(ref desc) = def.description {
            format!("{key} ({desc})")
        } else {
            key.to_string()
        };
...
static TEMPLATE_SKILL_TOML: &str = r#"[skill]
name = "SKILL_NAME"
version = "0.1.0"
...
static TRANSFORM_SKILL_TOML: &str = r#"[skill]
name = "SKILL_NAME"
version = "0.1.0"
description = "TODO: describe what this skill does"
...
# file = ".gitignore"
# operation = "line_append"
# skip_duplicates = true
# lines = ["TODO: add your entries"]

# Example: merge into a JSON file
# [[skill.execution.transforms]]
# file = "package.json"
